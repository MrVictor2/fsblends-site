...
const candlesClassic = products.filter(
  (p) => p.type === "candle" && p.collection === "classic"
);
const candlesDessert = products.filter(
  (p) => p.type === "candle" && p.collection === "dessert"
);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-[clamp(1.5rem,4vw,3rem)]">
      ...
      <p className="mt-1 text-neutral-700">
        Browse all FS Blends products by category.
      </p>
...
/** Reusable grid section (with fixed square thumbnails) */
function Section({
  id,
  title,
  items,
          ...
          return (
            <Link key={href} href={href} className="block group">
              <Card className="overflow-hidden bg-white/80 transition hover:shadow-md">
                {/* Square thumbnail for consistent card heights */}
